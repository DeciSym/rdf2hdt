...
pub fn build_hdt(
    ...
    dest_file: &str,
    opts: Options,
) -> Result<ConvertedHDT, Box<dyn Error>> {
...
impl ConvertedHDT {
    ...
    fn build_header(&mut self, source_file: &str) -> Result<(), Box<dyn Error>> {
        ...
        header.insert(Triple::new(
            ...
            Literal::new_simple_literal(
                (self.dict.object_terms.len() + self.dict.shared_terms.len()).to_string(),
            ),
        ));
        ...
        header.insert(Triple::new(
            ...
            Literal::new_simple_literal("FIXME"),
        ));
...
mod tests {
    ...
    fn test_build_hdt() {
        ...
        assert_eq!(
            ...
            conv_hdt.dict.subject_terms.len()
        );
        assert_eq!(
            dict.predicates.num_strings(),
            conv_hdt.dict.predicate_terms.len()
        );
